...
#    filedir = 'limit_cycle_surface'
    ...
    filename = 'phase_yz_plane_inproportonate_imag_vals'
    filedir = ''
    ...
    _a = (1.0 + 1.0j)
    _b = (-1.0 + 30.0j)
#    _b = -complex( np.power( _b , 1/4 ) )
    print( 'alpha:' , _a , '     beta:' , _b , '\n' )
#    exit(0)
...
#    ax.plot_surface( px , py , pz , alpha=0.5 , color = '#0099ff') # Light-Blue
    ...
    re_a , re_b = abs( _a.real ) , abs( _b.real )
    for c in circs:
        r = np.sqrt( re_a * c / re_b )
        cy = np.cos( cyrng )*r
        cz = np.sin( czrng )*r
        cx = np.ones_like( cy )*c
        ax.plot( cx , cy , cz , 'k' , linewidth=_linwdth , alpha=0.5 )   # for circles
    ...
    zeros = [0, 0]
    lim = 2.0
    ax.plot( [-lim, 0.] , zeros , zeros , 'k-' , linewidth=_linwdth ) # , alpha=0.5 )
    ax.plot( [ 0., 1.0 ] , zeros , zeros , 'k--' , linewidth=_linwdth ) # , alpha=0.5 )
    ax.plot( [ 1.0, lim ] , zeros , zeros , 'k-' , linewidth=_linwdth ) # , alpha=0.5 )
    dsh = 1.05
    ax.plot( zeros , [-lim,-dsh] , zeros , 'k-' , linewidth=_linwdth ) #, alpha=0.5 )
    ax.plot( zeros , [-dsh,dsh] , zeros , 'k--' , linewidth=_linwdth ) #, alpha=0.5 )
    ax.plot( zeros , [dsh,lim] , zeros , 'k-' , linewidth=_linwdth ) #, alpha=0.5 )
    ax.plot( zeros , zeros , [-lim,-dsh] , 'k-' , linewidth=_linwdth ) #, alpha=0.5 )
    ax.plot( zeros , zeros , [-dsh,dsh] , 'k--' , linewidth=_linwdth ) #, alpha=0.5 )
    ax.plot( zeros , zeros , [dsh,lim] , 'k-' , linewidth=_linwdth ) #, alpha=0.5 )
    ax.view_init( elev=0 , azim=0 , roll=0 ) # YZ Axis
    #'''
