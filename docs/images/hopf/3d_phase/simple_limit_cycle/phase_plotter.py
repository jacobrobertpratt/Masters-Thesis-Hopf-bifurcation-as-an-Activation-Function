...
def cpx_hopf_step_real( z , a , b ):
    zz = np.matmul( z.transpose((0,1,2,4,3)) , z )
    bzz = np.matmul( b , zz )
    dz = np.matmul( ( a + bzz ) , z )
    return dz
...
#    filedir = 'limit_cycle_surface'
    ...
    filename = 'simple_limit_cycle_adjusted'
    filedir = ''
    ...
    ons , zrs = np.ones_like( gz ) , np.zeros_like( gz )
    z = np.asarray( [[gy,-gz],[gz,gy]] ).transpose((2,3,4,1,0))
    b = np.asarray( [[ons,ons],[ons,ons]] ).transpose((2,3,4,1,0)) * _b
    a = np.asarray( [[ons,ons],[ons,ons]] ).transpose((2,3,4,1,0)) * _a
    
    dz = cpx_hopf_step_real( z , a , b )
    print( 'dz:\n', dz , '\nshape:' , dz.shape , '\n' )
    
    dy = dz[::,::,::,0,0] # ( dz[::,::,::,0,0] + dz[::,::,::,1,1] ) / 2.
    dz = dz[::,::,::,0,1] # np.sqrt( dz[::,::,::,0,0]*dz[::,::,::,1,1] - dz[::,::,::,0,1]*dz[::,::,::,1,0] ) / 4.
#    print( 'dy:\n', dy , '\nshape:' , dy.shape , '\n' )
#    print( 'dz:\n', dz , '\nshape:' , dz.shape , '\n' )
#    exit(0)
